...
Lumen is architected from the ground up as a payment-ready, on-chain memory primitive for Base. While our immediate release focuses on local-first Sibyl substrate execution, the x402 payment specification is directly integrated into our roadmap: exposing `/brief` as an x402-gated micro-payment endpoint where external agent swarms pay fractions of a cent in USDC on Base to query high-signal learned patterns. This architectural choice positions Lumen to evolve from an internal outcome layer into an open on-chain market for distilled agent competence on Base mainnet.
